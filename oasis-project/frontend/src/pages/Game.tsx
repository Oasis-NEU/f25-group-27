...
export default function StreetViewApp() {
  ...
  useEffect(() => {
    ...
    script.src = `https://maps.googleapis.com/maps/api/js?key=${API_KEY}&libraries=marker,geometry&loading=async&callback=initMap`;
    script.async = true;
    script.defer = true;
            ...
            onClick={() => {
              ...
              if (markerRef.current && markerRef.current.position) {
                ...
                const calculatedScore = Math.round(
                  Math.min(
                    1000,
                    Math.max(0, 1000 * Math.exp((-3 * difference) / 1000) + 50)
                  )
                );
                ...
                if (mapInstanceRef.current) {
                  ...
                  actualLocationMarkerRef.current =
